...
By utilising Hyperledger Fabric as the consortium infrastructure, the framework provides a permissioned, deterministically executable environment with certificate-based access control that balances the transparency of blockchain with the strict confidentiality and access-control requirements of regulated financial institutions. (On the single-host test deployment, measured consensus latency is $2115.9$\,ms per transaction and measured peak goodput is $40.3$\,tps at client concurrency $10$; both are bounded by the orderer's default $2$\,s \texttt{BatchTimeout} and by Fabric's MVCC validation, so production-grade throughput would require a tuned multi-host network and is out of scope --- see Limitations.)
...
On the consensus path, DB-BOA-driven leader selection aims to reduce simulated block latency relative to random selection. On the learning side, the federated ADTCN aims for robust fraud-detection performance on the extremely imbalanced ULB data, measured by MCC and balanced accuracy rather than raw accuracy (which is uninformative at 0.17\% fraud). Per-round latency and sustained throughput are both measured on a real single-host, two-organisation Fabric deployment (mean $2115.9$\,ms; peak $40.3$\,tps), not on a live multi-host network; only the leader-\emph{selection frequencies} come from the simulation layer's node resource scores. They characterise relative behaviour rather than production performance (see Limitations).
...
On the consensus path, the objective is to reduce simulated block latency relative to random leader selection through DB-BOA-driven leader scheduling. On the detection side, the goal is a working federated detector obtained without manual tuning; the verified detector averages MCC $0.753 \pm 0.055$ over five seeds at the weight-DP-off deployment operating point, the single deployment run itself scoring $0.677$ (Chapter~\ref{ch:results}). Note that this is not a measured improvement on a carefully hand-tuned default either: the difference between them is $+0.047$ MCC at $p=0.34$, i.e.\ indistinguishable from zero at five seeds. DB-BOA's demonstrated contribution is therefore automation and leader selection, not a detection-accuracy gain (reported honestly in Chapter~\ref{ch:results}).
...
Leader selection runs over a ten-node simulated consortium, where DB-BOA minimises the reputation-discounted resource cost each round. The selection frequencies reported by this layer are computed from node resource scores in simulation (\texttt{leader\_block.py}); the per-round consensus latency and the throughput figure are taken from a real single-host, two-organisation Fabric measurement (mean $2115.9$\,ms; peak $40.3$\,tps at concurrency $10$), not from a live multi-host network. The frequencies therefore characterise the relative behaviour of reputation-aware selection, while the timing figures are measured but hardware- and configuration-bound; both are revisited in the Limitations.
...
At the weight-DP-off deployment operating point, the DB-BOA-tuned ADTCN averages MCC $0.753 \pm 0.055$ across five training seeds on the held-out ULB test set, with the single deployment run --- the one the reported confusion matrix and ROC curve come from --- scoring $0.677$ (Chapter~\ref{ch:results}). These are honest numbers for a lightweight one-dimensional CNN on raw PCA features; they are not the base paper's 95.45\% nor an inflated 99.31\% — those are not produced by this implementation.
