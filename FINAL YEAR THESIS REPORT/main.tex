...
\setlength{\emergencystretch}{2em} % let TeX loosen lines instead of overflowing the margin (unbreakable \texttt/slash tokens)
...
\usepackage{amssymb} % blackboard-bold (\mathbb) and relation symbols (\gtrsim)
...
\usepackage{subcaption}   % ✅ ADD HERE
\usepackage{float}
...
\usepackage{booktabs} % Provides \toprule, \midrule, \bottomrule for professional tables
\usepackage{placeins} % Provides \FloatBarrier to keep floats within their section
...
\input{core/nomenclature} % nomenclature entries (no visible output here; rendered by \printnomenclature)
